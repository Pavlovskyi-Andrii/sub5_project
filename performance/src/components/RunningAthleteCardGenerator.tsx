...
import { calculateRunningAthleteProfile, Gender } from '@/utils/runningAthleteCalculations';
...
import avatarRunnerMaleBald from '@/assets/avatar-runner-male-bald.png';
import avatarRunnerFemaleBlond from '@/assets/avatar-runner-female-blond.png';
import avatarRunnerFemaleBrown from '@/assets/avatar-runner-female-brown.png';
import avatarRunnerFemaleDark from '@/assets/avatar-runner-female-dark.png';

interface RunningAthleteCardGeneratorProps {
  models: PrecisionModels;
}

const getMaleRunnerAvatars = () => [
  { id: 'runner-male-blond', src: avatarRunnerMaleBlond, name: 'Biondo' },
  { id: 'runner-male-brown', src: avatarRunnerMaleBrown, name: 'Castano' },
  { id: 'runner-male-bald', src: avatarRunnerMaleBald, name: 'Calvo' }
];

const getFemaleRunnerAvatars = () => [
  { id: 'runner-female-blond', src: avatarRunnerFemaleBlond, name: 'Bionda' },
  { id: 'runner-female-brown', src: avatarRunnerFemaleBrown, name: 'Castana' },
  { id: 'runner-female-dark', src: avatarRunnerFemaleDark, name: 'Mora' }
];
...
const RunningAthleteCardGenerator: React.FC<RunningAthleteCardGeneratorProps> = ({
  ...
  // Calcola il profilo dell'atleta runner
  const runnerProfile = calculateRunningAthleteProfile(models, gender);

  // Avatar dinamici basati sul genere
  const availableAvatars = gender === 'male' ? getMaleRunnerAvatars() : getFemaleRunnerAvatars();
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        <DialogContent className="sm:max-w-6xl bg-slate-800 border-slate-700 max-h-[85vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            {/* Form di input - colonna più stretta */}
            <Card className="bg-slate-900 border-slate-700 lg:col-span-2">
              <CardHeader className="pb-3">
                <CardTitle className="text-lime-400 text-lg">Personalizza la tua Card</CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                {/* Nome Atleta */}
                <div>
                  <Label className="text-slate-200 text-sm">Nome Atleta</Label>
                  <Input
                    value={athleteName}
                 ...
                 {/* Sesso */}
                 <div>
                   <Label className="text-slate-200 text-sm">Sesso</Label>
                   <RadioGroup 
                     value={gender} 
                 ...
                 {/* Nazionalità */}
                <div>
                  <Label className="text-slate-200 text-sm">Nazionalità</Label>
                ...
                {/* Immagine profilo */}
                <div>
                  <Label className="text-slate-200 text-sm">Immagine Profilo</Label>
                  ...
                  {/* Avatar predefiniti */}
                  <div className="mt-3">
                    <Label className="text-slate-300 text-xs">O scegli un avatar ({gender === 'male' ? 'maschile' : 'femminile'}):</Label>
                ...
                {profileImage && (
                  <div className="mt-3">
                    <Label className="text-slate-200 text-sm">Anteprima</Label>
                    <div className="mt-2 w-16 h-16 rounded-full overflow-hidden border-2 border-lime-400">
                      <img
                        src={profileImage}
                        alt="Preview"
                ...
                <div className="flex space-x-2 pt-2">
                  <Button
                    onClick={resetForm}
                    variant="outline"
                    className="flex-1 border-slate-600 text-slate-200 hover:bg-slate-700 h-8 text-sm"
                  >
                    Reset
                  </Button>
                  <Button
                    ...
                    className="flex-1 bg-lime-400 hover:bg-lime-500 text-slate-900 font-semibold h-8 text-sm"
                  >
...
export default RunningAthleteCardGenerator;
